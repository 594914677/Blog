layui.config({
    ...
    var form = layui.form,
        ...
        $ = layui.jquery;
    var news = $(".news").attr('data-url');

    //创建一个编辑器
    var editIndex = layedit.build('news_content',{
        uploadImage: {url: news, type: 'post'}
    });
    //执行一个laydate实例
    laydate.render({
        elem: '.newsTime' //指定元素
    ...
    //监听提交
    form.on('submit(upNews)', function(data){
        ...
        $.post("newsUp",
            {newsId:newsId,newsName:newsName,tuijian:tuijian,newsStatus:newsStatus,isShow:isShow,newsAuthor:newsAuthor,newsTime:newsTime,newsCid:newsCid,keyword:keyword,textarea:textarea,content:content},
            function(res){
                if(res.status){
